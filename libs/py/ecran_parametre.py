...
from kivymd.uix.toolbar import MDTooltip
...
class MonButtonTheme(MDCard):
	md_bg_color = ColorProperty([0.12941176470588237, 0.5882352941176471, 0.9529411764705882, 1.0])  # couleur par défaut
...
class EcranParam(Screen, BoxLayout):
	...
	def charger_app(self):
		"""
		Cette fonction est faite uniquement pour pouvir utiliser des fonction qui se trouve dans le fichier main.py
		dans ce fichier.
		On peut les appeler dans les fichier kv a partire de app, mais on ne peut les appeler dans les fichier py.
		Du coup, je charge la proprieté app du .kv dans le .py pour pouvoir l'utiliser.
		"""
		self.apps.theme_cls.theme_style = "Dark" if self.apps.theme_cls.theme_style == "Light" else "Light"
