...
class MainScreen(Screen, BoxLayout):
	...
	definition = ""
	# fermer = ""
	couleur = None  # Couleur du bouton aimer
	menu = None
	numero = 0
	...
	def __init__(self, **kwargs):
		...
		# permet de lier la definition a la fonction 'on_clic_ref'
	...
	def on_clic_ref(self, instance, ref_label: str) -> None:
		...
		:return: None
		"""
		# if self.dialog_ref.title != ref_label:

		font_name = self.ids.id_kv_definition.font_name
		font_size = self.ids.id_kv_definition.font_size
		try:
			ContenRef.definition = purge(
				interroger_bd("definition", "dictionnaire", f'WHERE idmot = "{ref_label.lower()}"')[0][0]
			)
		except Exception as e:
			print(e)
			toast(text=f"Impossible d'afficher '{ref_label}', une erreur c'est produite", duration=4)
			return
		...
		self.dialog_ref.ids.container.padding = ["10dp", "10dp", "-5dp", "10dp"]
		...
		if self.dialog_ref.ids.title.font_name != font_name or self.dialog_ref.ids.title.font_size != font_size:
			...
			try:
				for i in self.dialog_ref.children[0].children[2].children[0].ids.values():
					# if i in ["nom_auteur", "mail_auteur", "url_auteur", "licence", "fermer"]:
	...
	def aimer(cls, button_aimer: bool = False) -> None:
		...
		try:
			existe: list = interroger_bd("id_mot", "aimer ", f'WHERE id_mot = "{idmot}"')[0][0]
		except Exception:
			existe = []
		...
		if existe == idmot:  # si le mot existe
			if button_aimer:
				...
				cls.couleur.text_color = screen_manager.get_screen("main_screen").ids.chevron_left.text_color
			# print(f"le btn est press val = {cls.couleur_def}")
			else:
