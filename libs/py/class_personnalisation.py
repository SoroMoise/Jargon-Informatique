...
class CustomSeparator(MDSeparator):
...
class ContentNavigationDrawer(BoxLayout):
    ...
    def mot_definition(cls, text):
        ...
        debut = interroger_bd("mot", "dictionnaire",
                              f'WHERE idmot like "{text}%" LIMIT 150')
        tout = []
        long = len(debut)
        if long < 150:
            tout = interroger_bd("mot", "dictionnaire ",
                                 f'WHERE idmot like "%{text}%" LIMIT {150-long}')
    ...
    def voir_list_mots(self, text="", recherche=False, button_recherche=False) -> None:
        ...
        if button_recherche and self.choix:
            self.ids.rv.data = []
